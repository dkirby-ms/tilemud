...
function normalizePlacementRules(rules) {
    if (!rules) {
        return DEFAULT_PLACEMENT_RULES;
    }
    const adjacency = rules.adjacency === "none" || rules.adjacency === "any" || rules.adjacency === "orthogonal"
        ? rules.adjacency
        : "orthogonal";
    return {
        adjacency,
...
function extractRequestId(action) {
...
export class ActionHandlerService {
    ...
    async handle(action, context) {
        ...
        switch (action.type) {
            ...
            default: {
                const error = new Error(`Unsupported action type: ${String(action.type)}`);
                return this.reject(action, "unknown", error, requestId);
            }
        }
    }
    async handleTilePlacement(action, state, requestId) {
        ...
        try {
            ...
            if (error instanceof TilePlacementValidationError) {
                ...
                if (conflictIssue) {
                    ...
                    const conflictError = new TileMudError("PRECEDENCE_CONFLICT", {
    ...
    handleScriptedEvent(action, state) {
        const tick = action.requestedTick ?? action.timestamp;
        state.tick = Math.max(state.tick, tick);
        const effect = {
            type: "scripted_event",
    ...
    apply(action, effects, tick, requestId) {
        return {
            status: "applied",
            action,
    ...
    reject(action, reason, error, requestId, details) {
        return {
            status: "rejected",
            action,
            instanceId: action.instanceId,
    ...
    async resolvePlacementRules(version) {
        const cached = this.placementRulesCache.get(version);
        if (cached) {
            return cached;
        }
        const detail = await this.rulesetService.requireRuleSetByVersion(version);
