...
async function run(config) {
    ...
    for (let i = 0; i < config.clients; i++) {
        if (Date.now() > deadline)
            throw new Error("harness timeout while joining");
        const client = new Client(config.endpoint);
        const ctx = { client, handshakeStart: performance.now(), snapshotSamples: [] };
        contexts.push(ctx);
...
async function joinBattleRoom(ctx, config) {
...
//# sourceMappingURL=latency-harness.js.map