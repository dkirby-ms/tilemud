...
const DEFAULT_ADJACENCY = "orthogonal";
const MAX_TAGS = 32;
...
export class RuleSetService {
    ...
    async getRuleSetByVersion(version) {
        const found = await this.repository.findByVersion(version);
        return found ? this.mapRuleSet(found) : null;
    }
    async requireRuleSetByVersion(version) {
...
function sanitizeInitialTiles(raw, width, height) {
...
function normalizeAdjacency(value) {
    if (!value) {
...
function sanitizeExtras(record, recognizedKeys) {
    const extraEntries = {};
...
function normalizeRuleSetMetadata(metadata) {
...
function cloneMetadata(metadata) {
    return {
        ...
        placement: { ...metadata.placement },
        extras: sanitizeJsonValue(metadata.extras)
    };
}
export function createRuleSetService(repository) {
