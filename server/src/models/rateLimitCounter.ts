...
export interface RateLimitCounterOptions {
  redis: Redis;
...
export class RateLimitCounter {
  ...
  constructor(options: RateLimitCounterOptions) {
  ...
  async getAllLimits(
    identifier: string,
    keyPrefix?: string
  ): Promise<Array<{ channel: string; count: number; resetTimeMs: number }>> {
    const prefix = keyPrefix || this.defaultConfig.keyPrefix!;
    const pattern = `${prefix}${identifier}:*`;
    const keys = await this.redis.keys(pattern);
    
    const results: Array<{ channel: string; count: number; resetTimeMs: number }> = [];
  ...
  async cleanupExpiredLimits(
    ...
    do {
      ...
      if (keys.length > 0) {
        ...
        const results = await pipeline.exec();
        if (results) {
          deletedCount += results.reduce((sum, [, count]) => sum + (count as number || 0), 0);
        }
      }
    } while (cursor !== 0);
  ...
  private buildKey(identifier: string, channel: string, keyPrefix: string): string {
...
export class GameRateLimits {
  ...
  async checkActionLimit(playerId: string): Promise<RateLimitResult> {
  ...
  async checkChatLimit(playerId: string): Promise<RateLimitResult> {
  ...
  async checkPrivateMessageLimit(playerId: string): Promise<RateLimitResult> {
  ...
  async checkInstanceCreationLimit(playerId: string): Promise<RateLimitResult> {
  ...
  async resetPlayerLimits(playerId: string): Promise<void> {
    await Promise.all([
  ...
  async getPlayerLimitStatus(playerId: string): Promise<Record<string, RateLimitResult>> {
  ...
  private async getRemainingOnly(
    playerId: string, 
    channel: string, 
    config: Partial<RateLimitConfig>
  ): Promise<RateLimitResult> {
...
export function createRateLimitCounter(redis: Redis, config?: Partial<RateLimitConfig>): RateLimitCounter {
...
export function createGameRateLimits(redis: Redis): GameRateLimits {
