...
1. **Given** an authenticated user with at least one existing character, **When** they open the character selection view and choose a character, **Then** the system establishes a game session using that character and displays confirmation that they are connected (e.g., shows other player chat stream) and they can submit a chat message which is visible to other connected players.
